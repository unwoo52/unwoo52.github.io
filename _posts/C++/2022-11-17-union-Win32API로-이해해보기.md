...
정확하게는 모르겠어도 STRICT가 항상 1(true)를 정의한다고 생각할 수 있을 것 같다. 이제는 구분되지 않는 과거의 잔재인듯.
...
1학년 때 컴퓨터과학 수업에서 컴퓨터의 역사 항목에서 보던 컴퓨터였다. 아마 우리 주변의 소프트웨어를 이루는 근간 이 되는 다른 코드들에 접근하면 이런 주석들이 더 많지 않을까? 깊숙히 숨은 다른 코드들도 궁금해진다.

### 전처리기

다시 minwindef.h로 돌아와서 코드를 구경한다. 이번엔 전처리기가 걸린다. 전에 배웠던 기억이 있지만 대충 이해하고 지나갔었기에 전처리기들에 대해 다시 검색해서 공부해보았다.. 
